...
::.zmrineuro::
    ...
    Clipboard := Clipboard . "[ lumbar ][ cervical ][ thoracic ]spine given the fact"
    if (gender = "m") {
...
::.zmrinormal::
    OldClipboard := Clipboard
    Clipboard := ""
    Clipboard := Clipboard . "I favor getting an MRI of the [ cervical ][ lumbar ][ thoracic ]spine to find the etiology of"
    if (gender = "m") {
        Clipboard := Clipboard . " his "
    } else {
        Clipboard := Clipboard . " her "
    }
    Clipboard := Clipboard . "complaints before we proceed into [ cervical ][ lumbar ]facet joint diagnostics and therapeutics."
    SendInput ^v
    Sleep, 50
    Clipboard := OldClipboard
return
...
::.zisis::
    OldClipboard := Clipboard
    Clipboard := ""
    Clipboard := Clipboard . "Specifically, I reviewed in detail with this patient, the Internation Spinal Intervention Society (ISIS) diagnostic and therapeutic paradigm for facet joint spondylosis. We reviewed the value of [ cervical ][ lumbar ]medial branch nerve block proven facet spondylosis for longer term palliation with RF neurolysis of the same. We reviewed procedural technique, potential risks, and reasonable expectations."
    SendInput ^v
    Sleep, 50
    Clipboard := OldClipboard
return
...
::.zsirepeat::
    OldClipboard := Clipboard
    Clipboard := ""
    Clipboard := Clipboard . "We discussed the possibility of repeating epiradicular steroid administrations [ at another level ][ at the same level ]to see if we could palliate and further refine the diagnosis. We reviewed again in detail with"
    if (gender = "m") {
        Clipboard := Clipboard . " him "
    } else {
        Clipboard := Clipboard . " her "
